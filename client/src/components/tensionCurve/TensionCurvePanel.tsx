    ...
    <Card className={cn("border-border/70 bg-background/95", compact ? "rounded-lg" : "rounded-xl")}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline">{t("panel.readonly")}</Badge>
              {userAnchorCount > 0 ? <Badge variant="secondary">{t("common.manualPinnedCount", { count: userAnchorCount })}</Badge> : null}
            </div>
      ...
      <CardContent className="space-y-3 pt-0">
        ...
        {!compact && canvasWidth > 900 ? (
          <div className="text-xs text-muted-foreground">{t("panel.canvasHint")}</div>
        ) : null}
