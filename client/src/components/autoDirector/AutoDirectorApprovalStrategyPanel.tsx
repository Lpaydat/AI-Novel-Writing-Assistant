...
export default function AutoDirectorApprovalStrategyPanel({
  ...
  return (
    <div className="mt-3 min-w-0 rounded-md border border-primary/15 bg-primary/5 p-3">
      <div className="text-xs font-medium text-foreground">{t("approvalStrategy.title")}</div>
      <div className={AUTO_DIRECTOR_MOBILE_CLASSES.approvalStrategyGrid}>
        ...
        >
          <div className="text-sm font-medium text-foreground">{t("approvalStrategy.autoMode.title")}</div>
          <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            {t("approvalStrategy.autoMode.description")}
          </div>
        </button>
        <button
          type="button"
        ...
        >
          <div className="text-sm font-medium text-foreground">{t("approvalStrategy.copilotMode.title")}</div>
          <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            {t("approvalStrategy.copilotMode.description")}
          </div>
        </button>
      </div>
      ...
      <div className={`mt-3 rounded-md border bg-background/80 p-3 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
        {enabled
          ? t("approvalStrategy.autoBoundary")
          : t("approvalStrategy.copilotBoundary", { summary: summarizeDirectorAutoApprovalPoints(approvalPointCodes) })}
      </div>

      {!enabled ? (
        <details className="mt-3 rounded-md border bg-background">
          <summary className="cursor-pointer px-3 py-2 text-xs font-medium text-foreground">
            {t("approvalStrategy.advancedTitle")}
          </summary>
