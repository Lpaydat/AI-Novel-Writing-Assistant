...
export default function TitleLibraryPanel({ genreOptions }: TitleLibraryPanelProps) {
  ...
  const markUsedMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.titles.all });
      toast.success(t("library.toast.usedUpdated"));
    },
  });

  const handleCopy = async (title: string) => {
    await navigator.clipboard.writeText(title);
    toast.success(t("library.toast.copied"));
  };
    ...
    <div className="space-y-5">
      <div className="grid gap-3 border-b border-border/60 pb-5 md:grid-cols-[minmax(0,1fr)_220px_180px]">
        <label className="space-y-2 text-sm">
          <span className="font-medium text-foreground">{t("library.search.label")}</span>
          <Input
            value={search}
        ...
        <label className="space-y-2 text-sm">
          <span className="font-medium text-foreground">{t("library.genre.label")}</span>
          <SelectControl
        ...
        <label className="space-y-2 text-sm">
          <span className="font-medium text-foreground">{t("library.sort.label")}</span>
          <SelectControl
            className={selectClassName}
            value={sort}
          ...
          >
            ...
            <option value="hot">{t("library.sort.hot")}</option>
            <option value="clickRate">{t("library.sort.clickRate")}</option>
          </SelectControl>
        </label>
      </div>
      ...
      {!libraryQuery.isLoading && rows.length === 0 ? (
        <div className="py-10 text-center">
          <div className="text-sm font-medium text-foreground">{t("library.empty.title")}</div>
          ...
          <div key={entry.id} className="group py-4 transition hover:bg-muted/[0.18]">
            <div className="grid gap-3 px-2 lg:grid-cols-[64px_minmax(0,1fr)_auto] lg:items-start">
              <div className="text-xs leading-5 text-muted-foreground">
                <div className="font-medium text-foreground">{t("library.estimate")}</div>
              ...
              <div className="min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                  ...
                  <span>{t("library.usedCount", { used: entry.usedCount })}</span>
              ...
              <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                ...
                >
                  ...
                  {markUsedMutation.isPending && markUsedMutation.variables === entry.id ? t("library.updating") : t("library.adopt")}
                </Button>
                <Button
                  type="button"
                  size="sm"
                  ...
                  onClick={() => {
                    const confirmed = window.confirm(t("library.confirmDelete", { title: entry.title }));
                    if (confirmed) {
                ...
                >
                  ...
                  {deleteMutation.isPending && deleteMutation.variables === entry.id ? t("library.deleting") : t("library.delete")}
                </Button>
              </div>
            </div>
          </div>
