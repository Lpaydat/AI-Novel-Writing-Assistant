...
export default function AntiAiRulesPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : t("page.toastCreateFailed")),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : t("page.toastSaveFailed")),
  });
  ...
  const aiDraftMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (!result) {
        toast.error(t("page.toastNoDraft"));
        return;
      }
      setForm({
      ...
      toast.success(t("page.toastDraftFilled"));
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : t("page.toastDraftFailed")),
  });
  ...
  const detectionMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : t("page.toastDetectFailed")),
  });
  ...
  const rewriteMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : t("page.toastRewriteFailed")),
  });
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (!payload.key || !payload.name || !payload.description) {
      toast.error(t("page.toastValidation"));
      return;
    }
    if (editingRule) {
