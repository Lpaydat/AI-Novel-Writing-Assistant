      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{t("test.testRules")}</div>
            <div className="mt-1 font-semibold">{totalRuleCount}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{t("test.temporary")}</div>
        ...
        {props.previewRules.length > 0 ? (
          <div className="space-y-2 rounded-md border bg-muted/20 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-medium text-foreground">{t("test.tempRulesTitle")}</div>
        ...
        {props.report ? (
          <div className="space-y-3 rounded-md border p-4">
            <div className="space-y-1">
              <div className="font-medium text-foreground">{t("test.riskScore")}{props.report.riskScore}</div>
              <div className="text-sm leading-6 text-muted-foreground">{props.report.summary}</div>
              <div className="text-xs text-muted-foreground">{t("test.hitRules")}{props.report.appliedRuleIds.length}</div>
            </div>
              ...
              <div className="space-y-2">
                {props.report.violations.map((item, index) => (
                  <div key={`${item.ruleId}-${index}`} className="rounded-md border bg-muted/20 p-3">
                    ...
                    <div className="mt-2 text-xs leading-5 text-muted-foreground">{t("test.suggestion")}{item.suggestion}</div>
                  </div>
                ))}
              </div>
            ) : (
        ...
        {props.rewritePreview ? (
          <div className="space-y-2">
            <div className="text-sm font-medium text-foreground">{t("test.rewritePreviewTitle")}</div>
