...
export default function KnowledgePage() {
  ...
  const clearFinishedRagJobsMutation = useMutation({
    ...
    onSuccess: async (response) => {
      setRagJobsActionMessage(response.message ?? t("page.clearedFinishedJobs"));
      await queryClient.invalidateQueries({ queryKey: ragJobsQueryKey });
    },
    onError: (error) => {
  ...
  const deleteRagJobMutation = useMutation({
    ...
    onSuccess: async (response) => {
      setRagJobsActionMessage(response.message ?? t("page.jobDeleted"));
      await queryClient.invalidateQueries({ queryKey: ragJobsQueryKey });
    },
    onError: (error) => {
  ...
  const handleUpload = async (file: File) => {
    if (!isTxtFile(file)) {
      throw new Error(t("page.onlyTxt"));
    }
    const content = await readTextFile(file);
    if (!content) {
      throw new Error(t("page.emptyFile"));
    }
  ...
  const handleClearFinishedRagJobs = () => {
    if (!window.confirm(t("page.confirmClearFinished"))) {
      return;
    }
