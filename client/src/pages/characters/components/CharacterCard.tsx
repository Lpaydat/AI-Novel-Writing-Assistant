  ...
  return (
    <div className="space-y-3 rounded-md border p-3">
      ...
      <div className="space-y-1 text-sm">
        <div><span className="text-muted-foreground">{t("card.personality")}</span>{character.personality || t("card.none")}</div>
        <div><span className="text-muted-foreground">{t("card.appearance")}</span>{character.appearance || t("card.none")}</div>
        <div><span className="text-muted-foreground">{t("card.weaknesses")}</span>{character.weaknesses || t("card.none")}</div>
        <div><span className="text-muted-foreground">{t("card.interests")}</span>{character.interests || t("card.none")}</div>
        <div><span className="text-muted-foreground">{t("card.keyEvents")}</span>{character.keyEvents || t("card.none")}</div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {!assetsLoading && assets.length === 0 ? (
          <div className="text-xs text-muted-foreground">{t("card.emptyAssets")}</div>
        ) : null}
        {assets.length > 0 ? (
            ...
            {assets.map((asset) => (
              <div key={asset.id} className="w-full max-w-[300px] space-y-2 rounded-md border p-2">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{asset.isPrimary ? t("card.primary") : t("card.candidate")}</div>
      ...
      >
        <AppDialogContent
          ...
          title={previewAsset ? t("card.imagePreviewTitle", { name: character.name }) : t("card.imagePreview")}
            ...
            <>
              <div className="flex max-h-[70vh] items-center justify-center overflow-auto rounded-md bg-muted/30 p-2">
                <img
                  ...
                  alt={t("card.previewAlt", { name: character.name })}
