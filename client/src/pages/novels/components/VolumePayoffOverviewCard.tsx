...
export default function VolumePayoffOverviewCard(props: VolumePayoffOverviewCardProps) {
  ...
  const chapterPayoffGroups = selectedVolume.chapters
    .map((chapter) => ({
      ...
      chapterTitle: chapter.title?.trim() || t("payoff.unnamedChapter"),
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <CardTitle className="text-base">{t("payoff.title")}</CardTitle>
            <div className="text-sm text-muted-foreground">
              {t("payoff.subtitle")}
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="outline">{t("payoff.volumeBadge", { order: selectedVolume.sortOrder })}</Badge>
            <Badge variant="outline">{t("payoff.openCount", { count: openPayoffRows.length })}</Badge>
            <Badge variant="outline">{t("payoff.linkedCount", { count: linkedOpenPayoffCount })}</Badge>
            <Badge variant={unplannedOpenPayoffs.length > 0 ? "secondary" : "outline"}>
              {t("payoff.unplannedCount", { count: unplannedOpenPayoffs.length })}
            </Badge>
            <Badge variant="outline">{t("payoff.arrangedCount", { count: chapterPayoffGroups.length })}</Badge>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="grid gap-3 xl:grid-cols-[minmax(280px,0.9fr)_minmax(0,1.35fr)]">
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="font-medium text-foreground">{t("payoff.openTitle")}</div>
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge variant={item.linkedChapters.length > 0 ? "default" : "secondary"}>
                        {item.linkedChapters.length > 0 ? t("payoff.arranged") : t("payoff.notArranged")}
                      </Badge>
                    </div>
                    {item.linkedChapters.length > 0 ? (
                      <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                        {item.linkedChapters.map((entry) => (
                          ...
                          >
                            {t("payoff.chapterRef", { order: entry.chapterOrder, title: entry.chapterTitle })}
                          </span>
                        ))}
                      </div>
                    ) : (
                      <div className="mt-2 text-xs text-muted-foreground">
                        {t("payoff.itemUnlinked")}
                      </div>
                    )}
                  </div>
                ))
          ...
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="font-medium text-foreground">{t("payoff.arrangeTitle")}</div>
              <Badge variant="outline">{chapterPayoffGroups.length}</Badge>
            </div>
            <div className="mt-1 text-xs text-muted-foreground">
              {t("payoff.arrangeHint")}
            </div>
            <div className="mt-3 space-y-2 text-sm">
                ...
                chapterPayoffGroups.map((chapter) => (
                  ...
                  >
                    <div className="font-medium text-foreground">
                      {t("payoff.chapterRef", { order: chapter.chapterOrder, title: chapter.chapterTitle })}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                      {chapter.refs.map((ref) => (
                        <span
