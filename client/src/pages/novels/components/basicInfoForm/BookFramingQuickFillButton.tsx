...
export function BookFramingQuickFillButton(props: BookFramingQuickFillButtonProps) {
  ...
  const suggestionMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("bookFramingQuickFill.autoFillFailed"));
    },
  });

  const handleGenerate = () => {
    if (!basicForm.title.trim() && !effectiveDescription) {
      toast.error(t("bookFramingQuickFill.needTitleOrDescription"));
      return;
    }
    if (hasExistingFramingContent(basicForm)) {
      const confirmed = window.confirm(t("bookFramingQuickFill.overwriteConfirm"));
      if (!confirmed) {
        return;
      }
    }
  ...
  return (
    ...
    >
      {suggestionMutation.isPending ? t("bookFramingQuickFill.filling") : t("bookFramingQuickFill.fillForMe")}
    </AiButton>
  );
}
