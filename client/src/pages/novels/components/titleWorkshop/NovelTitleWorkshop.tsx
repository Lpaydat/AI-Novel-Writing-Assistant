...
export default function NovelTitleWorkshop({
  ...
  const saveCurrentMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.titles.all });
      toast.success(t("titleWorkshop.currentSaved"));
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-1">
            <div className="text-sm font-semibold text-foreground">{t("titleWorkshop.title")}</div>
            <div className="text-sm leading-6 text-muted-foreground">
              {t("titleWorkshop.description")}
            </div>
          </div>
          <Button type="button" variant="outline" disabled={!currentTitle.trim() || saveCurrentMutation.isPending} onClick={() => saveCurrentMutation.mutate()}>
            {saveCurrentMutation.isPending ? t("titleWorkshop.saving") : t("titleWorkshop.saveCurrent")}
          </Button>
        </div>
        <div className="space-y-3">
          ...
          <div className="flex justify-end">
            <AiButton type="button" onClick={() => generateMutation.mutate()} disabled={generateMutation.isPending}>
              {generateMutation.isPending ? t("titleWorkshop.generating") : t("titleWorkshop.generateButton")}
            </AiButton>
          </div>
        </div>
      </div>
      ...
      <TitleSuggestionList
        ...
        primaryActionLabel={t("titleWorkshop.applyLabel")}
