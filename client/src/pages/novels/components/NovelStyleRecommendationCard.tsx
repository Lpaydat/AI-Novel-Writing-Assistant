...
export default function NovelStyleRecommendationCard({ novelId }: NovelStyleRecommendationCardProps) {
  ...
  const recommendMutation = useMutation({
    ...
    onError: (error) => {
      setMessage(error instanceof Error ? error.message : t("style.recommendError"));
    },
  });
  ...
  const applyMutation = useMutation({
    ...
    onSuccess: async () => {
      setMessage(t("style.appliedMessage"));
    ...
    onError: (error) => {
      setMessage(error instanceof Error ? error.message : t("style.bindError"));
    },
  });
      ...
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-1">
            <CardTitle>{t("style.cardTitle")}</CardTitle>
          ...
          <div className="flex flex-wrap gap-2">
            <Button asChild type="button" variant="outline">
              <Link to="/style-engine">{t("style.openLibrary")}</Link>
            </Button>
            <Button asChild type="button" variant="outline">
              <Link to="/style-engine?mode=imitate">{t("style.createNewStyle")}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,0.96fr)_minmax(0,1.04fr)]">
          <div className="space-y-4 rounded-2xl border bg-slate-50/70 p-4">
            <div className="rounded-2xl border bg-white p-4">
              <div className="text-sm font-medium text-slate-900">{t("style.currentBookStyle")}</div>
              {hasConfirmedBookStyle ? (
                  ...
                  {currentBindings.map((binding) => (
                    <div key={binding.id} className="rounded-xl border bg-slate-50/70 p-3">
                      ...
                      <div className="mt-1 text-xs text-slate-600">
                        {t("style.priorityStrength", { priority: binding.priority, weight: binding.weight })}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-3 text-sm leading-7 text-muted-foreground">
                  {t("style.noBindingHint")}
                </div>
              )}
            </div>

            <div className="rounded-2xl border bg-slate-950 p-4 text-white">
              <div className="text-sm font-medium">{t("style.effectTitle")}</div>
              <div className="mt-3 space-y-2 text-sm leading-7 text-slate-200">
                <div>{t("style.planningPhase")}</div>
                <div>{t("style.contentPhase")}</div>
              </div>
            </div>

            <div className="flex flex-wrap justify-end gap-2">
              <AiButton onClick={() => recommendMutation.mutate()} disabled={recommendMutation.isPending}>
                {recommendMutation.isPending ? t("style.recommendingStyle") : t("style.generateRecommendations")}
              </AiButton>
              {recommendation ? (
          ...
          <div className="space-y-4 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{t("style.resultTitle")}</div>
            {recommendation ? (
              <>
                    ...
                    {recommendation.candidates.map((candidate) => (
                      <div key={candidate.styleProfileId} className="rounded-2xl border bg-white p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge variant="outline">{t("style.fitScore", { value: candidate.fitScore })}</Badge>
                        </div>
                        ...
                        {candidate.caution ? (
                          <div className="mt-3 rounded-xl border bg-amber-50/70 p-3 text-xs leading-6 text-amber-900">
                            {t("style.cautionLine", { value: candidate.caution })}
                          </div>
                        ) : null}
                        <div className="mt-4 flex justify-end">
                          ...
                          >
                            {applyMutation.isPending ? t("style.bindingStyle") : t("style.setAsDefault")}
                          </Button>
                        </div>
                      </div>
                    ))}
