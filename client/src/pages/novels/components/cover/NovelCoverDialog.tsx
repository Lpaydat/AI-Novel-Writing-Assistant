...
const IMAGE_STATUS_TEXT_KEY: Record<string, string> = {
  queued: "coverDialog.status.queued",
  running: "coverDialog.status.running",
  succeeded: "coverDialog.status.succeeded",
  failed: "coverDialog.status.failed",
  cancelled: "coverDialog.status.cancelled",
};
...
export function NovelCoverDialog(props: NovelCoverDialogProps) {
  ...
  const currentSendModeLabel = promptMode === "direct"
    ? (directPromptSource === "optimized" ? t("coverDialog.sendMode.optimized") : t("coverDialog.sendMode.manual"))
    : t("coverDialog.sendMode.chain");
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      if (!imageForm.provider) {
        throw new Error(t("coverDialog.needImageProvider"));
      }
  ...
  const handleDeleteAsset = async (asset: ImageAsset) => {
    const confirmed = window.confirm(t("coverDialog.deleteConfirm"));
    if (!confirmed) {
      return;
    }
      ...
      <DialogContent className="flex max-h-[92vh] w-[96vw] max-w-[1120px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-0">
        <DialogHeader className="shrink-0 border-b border-slate-200 px-6 pb-4 pt-5">
          <DialogTitle className="text-[22px] font-semibold tracking-tight text-slate-900">
            {t("coverDialog.title")}
            {promptContext.title ? t("coverDialog.titleSuffix", { title: promptContext.title }) : ""}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
          {imageProviderOptions.length === 0 ? (
            <section className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              <div className="font-medium">{t("coverDialog.cannotStartTitle")}</div>
              <div className="mt-1 leading-6">
                {t("coverDialog.noProviderPrefix")}
                {" "}
                <Link className="font-medium underline underline-offset-2" to="/settings">
                  {t("coverDialog.settingsLink")}
                </Link>
                {" "}
                {t("coverDialog.noProviderSuffix")}
              </div>
            </section>
          ) : null}
          ...
          <section className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50/65 p-4">
            <div className="space-y-1">
              <div className="text-sm font-semibold text-slate-900">{t("coverDialog.sourceHeading")}</div>
              <div className="text-xs leading-5 text-slate-500">
                {t("coverDialog.sourceHint")}
              </div>
            </div>
            <textarea
              ...
              placeholder={t("coverDialog.sourcePlaceholder")}
              value={sourcePrompt}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{t("coverDialog.outputLanguageLabel")}</div>
              ...
              <div className="flex min-w-0 flex-col gap-3 xl:items-end">
                <div className="flex flex-wrap items-center gap-2 xl:flex-nowrap">
                  ...
                  >
                    {optimizeMutation.isPending ? t("coverDialog.optimizing") : t("coverDialog.optimizeButton")}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                  ...
                  >
                    {t("coverDialog.restoreChain")}
                  </Button>
                </div>

                <div className="flex flex-wrap items-center gap-2 text-sm xl:justify-end">
                  <span className="text-slate-500">{t("coverDialog.sendModeLabel")}</span>
          ...
          <section className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50/55 p-4">
            <div className="space-y-1">
              <div className="text-sm font-semibold text-slate-900">{t("coverDialog.finalPreviewHeading")}</div>
              <div className="text-xs leading-5 text-slate-500">
                {t("coverDialog.finalPreviewHint")}
              </div>
            </div>
            <textarea
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <input
              ...
              placeholder={t("coverDialog.stylePresetPlaceholder")}
              value={imageForm.stylePreset}
              onChange={(event) => updateStylePreset(event.target.value)}
            />
            <input
              className="rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200 xl:col-span-2"
              placeholder={t("coverDialog.negativePromptPlaceholder")}
              value={imageForm.negativePrompt}
              onChange={(event) => setImageForm((prev) => ({ ...prev, negativePrompt: event.target.value }))}
            />

            <label className="space-y-1 text-sm">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{t("coverDialog.providerLabel")}</div>
              <SelectControl
              ...
              >
                {imageProviderOptions.length === 0 ? (
                  <option value="">{t("coverDialog.selectProviderPlaceholder")}</option>
                ) : null}
            ...
            <label className="space-y-1 text-sm">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{t("coverDialog.sizeLabel")}</div>
              <SelectControl
                className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
              ...
              >
                <option value="1024x1536">{t("coverDialog.sizePortrait")}</option>
            ...
            <label className="space-y-1 text-sm">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{t("coverDialog.countLabel")}</div>
              <SelectControl
                className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
              ...
              >
                <option value="1">{t("coverDialog.imageCount", { count: 1 })}</option>
                <option value="2">{t("coverDialog.imageCount", { count: 2 })}</option>
                <option value="3">{t("coverDialog.imageCount", { count: 3 })}</option>
                <option value="4">{t("coverDialog.imageCount", { count: 4 })}</option>
              </SelectControl>
            </label>
            ...
            <div className="flex items-end">
              ...
              >
                {generateMutation.isPending ? t("coverDialog.submitting") : t("coverDialog.generate")}
              </Button>
            </div>
          </div>
          ...
          {optimizeMutation.isError ? (
            <div className="rounded-2xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive">
              {getErrorMessage(optimizeMutation.error, t("coverDialog.optimizeFailed"))}
            </div>
          ) : null}

          {generateMutation.isError ? (
            <div className="rounded-2xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive">
              {getErrorMessage(generateMutation.error, t("coverDialog.submitFailed"))}
            </div>
          ) : null}

          {activeTask ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
              <div>{t("coverDialog.taskStatus", { status: IMAGE_STATUS_TEXT_KEY[activeTask.status] ? t(IMAGE_STATUS_TEXT_KEY[activeTask.status]) : activeTask.status })}</div>
              {activeTask.error ? (
          ...
          <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="text-sm font-semibold text-slate-900">{t("coverDialog.galleryHeading")}</div>
                <div className="text-xs leading-5 text-slate-500">
                  {t("coverDialog.galleryHint")}
                </div>
              </div>
            </div>
            ...
            {assetsQuery.isLoading ? (
              <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500">
                {t("coverDialog.loadingGallery")}
              </div>
            ) : null}

            {!assetsQuery.isLoading && assets.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500">
                {t("coverDialog.emptyGallery")}
              </div>
            ) : null}
                ...
                {assets.map((asset) => (
                  <div key={asset.id} className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-3">
                    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                      <div className="aspect-[2/3] w-full">
                        <img
                          ...
                          alt={t("coverDialog.candidateAlt", { title: promptContext.title || t("coverDialog.novelFallback") })}
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        {asset.isPrimary ? t("coverDialog.currentPrimary") : t("coverDialog.candidate")}
                      </span>
                      <span className="text-xs text-slate-500">
                        {asset.width && asset.height ? `${asset.width} x ${asset.height}` : t("coverDialog.sizePending")}
                      </span>
                    </div>

                    {asset.localPath ? (
                      <div className="text-[11px] leading-5 text-slate-500 break-all">
                        {t("coverDialog.localPath", { path: asset.localPath })}
                      </div>
                    ) : null}
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {asset.isPrimary ? t("coverDialog.currentPrimary") : t("coverDialog.setAsPrimary")}
                      </Button>
                      <Button
                        type="button"
                        size="sm"
                        ...
                        onClick={() => {
                          void handleDeleteAsset(asset).catch((error) => {
                            window.alert(getErrorMessage(error, t("coverDialog.deleteFailed")));
                          });
                        }}
                      >
                        {deleteAssetMutation.variables === asset.id ? t("coverDialog.deleting") : t("coverDialog.delete")}
                      </Button>
                    </div>
                  </div>
                ))}
