...
function renderChapterDetailStatusBadge(chapter: StructuredChapter, t: TFunction) {
...
export default function StructuredChapterListCard(props: StructuredChapterListCardProps) {
  ...
  function renderBeatStatusBadge(group: typeof beatGroups[number]) {
    ...
    if (isGeneratingGroup) {
      return <Badge>{t("chapterList.status.generating")}</Badge>;
    }
    if (group.chapters.length === 0) {
      return <Badge variant="outline">{selectedVolumeChapters.length === 0 ? t("chapterList.status.pending") : t("chapterList.status.retry")}</Badge>;
    }
    if (group.expectedCount > 0 && group.chapters.length !== group.expectedCount) {
      return <Badge variant="outline">{t("chapterList.status.retry")}</Badge>;
    }
    return <Badge variant="secondary">{t("chapterList.status.done")}</Badge>;
  }
      ...
      <CardHeader className="pb-3">
        <div className="space-y-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <CardTitle className="text-base leading-none">{t("chapterList.title")}</CardTitle>
              <div className="mt-1 text-sm text-muted-foreground">
                {selectedBeat
                  ? t("chapterList.focusHint", { label: selectedBeat.label })
                  : t("chapterList.groupHint")}
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              >
                {isGeneratingCurrentVolume && generatingChapterListMode === "full_volume" ? t("common.generating") : t("chapterList.generateVolume")}
              </AiButton>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
            ...
            >
              {t("chapterList.allBeats")}
            </button>
            <Badge variant="outline">{t("chapterList.showChapters", { visible: visibleChapters.length, total: selectedVolumeChapters.length })}</Badge>
      ...
      <CardContent className="space-y-3 pt-0">
        {selectedVolumeNeedsChapterExpansion ? (
          <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs leading-6 text-amber-800">
            {t("chapterList.needExpansion", { current: selectedVolumeChapters.length, required: selectedVolumeRequiredChapterCount })}
          </div>
        ) : null}
        ...
        {selectedVolumeChapters.length > 0 ? (
            ...
            <div className="structured-chapter-navigation-list space-y-3 xl:max-h-[calc(100vh-12rem)] xl:overflow-y-auto xl:pr-1">
                      ...
                      >
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {t("chapterList.groupProgress", { done: group.chapters.length, total: Math.max(group.expectedCount, group.chapters.length, 1), refined: group.refinedCount })}
                          </span>
                        </div>
                      </button>
                      {active && selectedVolumeChapters.length > 0 ? (
                        ...
                        >
                          {isGeneratingCurrentVolume && generatingChapterListMode === "single_beat" && generatingChapterListBeatKey === group.key
                            ? t("chapterList.regen.loading")
                            : t("chapterList.regen.label")}
                        </AiButton>
                      ) : null}
                    </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                <Badge variant={isSelected ? "default" : "outline"}>{t("common.chapterLabel", { order: chapter.chapterOrder })}</Badge>
                                {renderChapterDetailStatusBadge(chapter, t)}
                              </div>
                              <div className="mt-2 text-sm font-medium">{chapter.title || t("common.chapterLabel", { order: chapter.chapterOrder })}</div>
                            </button>
                          );
                        }) : (
                          <div className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground">
                            {t("chapterList.groupEmpty")}
                          </div>
                        )}
                      </div>
                    ) : null}
              ...
              {unmatchedChapters.length > 0 ? (
                <div className="rounded-xl border border-dashed p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline">{t("chapterList.unmatched.badge")}</Badge>
                      <Badge variant="secondary">{t("common.chapterCount", { count: unmatchedChapters.length })}</Badge>
                    </div>
                    <span className="text-xs text-muted-foreground">{t("chapterList.unmatched.hint")}</span>
                  </div>
                  <div className="mt-3 space-y-2">
                    {unmatchedChapters.map((chapter) => {
                      const isSelected = selectedChapter?.id === chapter.id;
                      const title = chapter.title || t("common.chapterLabel", { order: chapter.chapterOrder });
                      return (
                        <div
                          key={chapter.id}
                          className={cn(
                        ...
                        >
                          ...
                          <Button
                            ...
                            title={t("chapterList.unmatched.deleteAria")}
                            onClick={() => {
                              const confirmed = window.confirm(t("chapterList.unmatched.deleteConfirm", { title }));
                              if (!confirmed) {
                                return;
                              }
                          ...
                          >
                            ...
                            <span className="sr-only">{t("chapterList.unmatched.deleteAria")}</span>
                          </Button>
                        </div>
                      );
                    })}
              ...
              <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
                {selectedVolumeNeedsChapterExpansion ? (
                  <div className="mb-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-6 text-amber-800">
                    {t("chapterList.beatNeedExpansion", { span: selectedBeat.chapterSpanHint, current: selectedVolumeChapters.length, required: selectedVolumeRequiredChapterCount })}
                  </div>
                ) : null}
                {t("chapterList.beatNoChapters")}
              </div>
            ) : null}
          </>
        ) : (
          <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
            {selectedVolumeRequiredChapterCount > 0 ? (
              <div className="mb-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-6 text-amber-800">
                {t("chapterList.requiredChapters", { required: selectedVolumeRequiredChapterCount })}
              </div>
            ) : null}
            {t("chapterList.noChapterList")}
          </div>
        )}
      </CardContent>
    </Card>
