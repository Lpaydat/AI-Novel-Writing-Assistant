  ...
  return (
    <div className="rounded-lg border bg-background/70 p-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm font-medium text-foreground">{t("chapterTargetSelector.advanceTo")}</div>
        <Input
          ...
          aria-label={t("chapterTargetSelector.advanceToAria")}
        />
      </div>
      <div className={`mt-2 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
        {t("chapterTargetSelector.rangeHint", { summary: target.summary, startOrder: target.startOrder, maxOrder: target.maxOrder })}
      </div>
    </div>
  );
}
