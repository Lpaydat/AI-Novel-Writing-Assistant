    ...
    <div className="min-w-0 rounded-xl border border-primary/20 bg-primary/5 p-3 sm:p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-2">
          <div className="text-sm font-medium text-foreground">{t("diagnosis.title")}</div>
          {isLoadingReadiness ? <Badge variant="outline">{t("diagnosis.loadingReadiness")}</Badge> : null}
          {readinessErrorMessage ? (
          ...
          {hasCurrentTask ? (
            ...
            <>
              ...
              <Button
                ...
                onClick={onStart}
              >
                {isStarting ? t("diagnosis.starting") : quickActionLabel}
              </Button>
            </>
          )}
          <div className={`text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            {t("diagnosis.retainAssetsHint")}
          </div>
        </div>
      </div>
    </div>
