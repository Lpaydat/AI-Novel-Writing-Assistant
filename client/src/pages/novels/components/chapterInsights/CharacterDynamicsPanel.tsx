...
function buildSnapshotCharacters(snapshot?: StoryStateSnapshot | null): SnapshotCharacterItem[] {
  ...
  return snapshot.characterStates.slice(0, 4).map((item, index) => {
    ...
    return {
      ...
      summary:
        parsedItem?.summary?.trim() ||
        item.summary?.trim() ||
        i18n.t("characterDynamics.summaryFallback", { ns: "novelsChapterInsights" }),
  ...
  return snapshot.relationStates.slice(0, 3).map((item, index) => {
    ...
    return {
      label:
        left && right
          ? `${left} → ${right}`
          : left || right || i18n.t("characterDynamics.relationFallbackLabel", { ns: "novelsChapterInsights" }),
      summary:
        parsedItem?.summary?.trim() ||
        item.summary?.trim() ||
        i18n.t("characterDynamics.relationSummaryFallback", { ns: "novelsChapterInsights" }),
    };
  });
}
  ...
  return snapshot.foreshadowStates.slice(0, 3).map((item, index) => {
    ...
    return {
      label:
        ...
        item.title ||
        i18n.t("characterDynamics.foreshadowing", { ns: "novelsChapterInsights" }),
      summary:
        parsedItem?.summary?.trim() ||
        item.summary?.trim() ||
        i18n.t("characterDynamics.foreshadowSummaryFallback", { ns: "novelsChapterInsights" }),
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-background p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {snapshot?.sourceChapterId ? <Badge variant="outline">{t("characterDynamics.sourceChapter")}</Badge> : null}
      </div>
      {snapshot ? (
        <div className="mt-2 space-y-3">
          ...
          {characters.length > 0 ? (
            <div className="space-y-2">
              <div className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("common.characterDynamics")}</div>
              {characters.map((item) => (
          ...
          {relations.length > 0 ? (
            <div className="space-y-2">
              <div className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("characterDynamics.sectionRelations")}</div>
              {relations.map((item) => (
          ...
          {foreshadows.length > 0 ? (
            <div className="space-y-2">
              <div className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("characterDynamics.foreshadowing")}</div>
    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-border/70 bg-background p-3">
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
          ...
          <span>{t("common.characterDynamics")}</span>
        </div>
        <div className="mt-2 text-sm font-medium text-foreground">{t("characterDynamics.subtitle")}</div>
        <div className="mt-1 text-xs leading-5 text-muted-foreground">{t("characterDynamics.hint")}</div>
      </div>

      {hasAnySnapshot ? (
        <div className="space-y-3">
          <CharacterSnapshotCard
            title={t("characterDynamics.chapterAfterState")}
            snapshot={chapterStateSnapshot}
            emptyText={t("characterDynamics.chapterAfterStateEmpty")}
          />
          <CharacterSnapshotCard
            title={t("common.latestState")}
            snapshot={latestStateSnapshot}
            emptyText={t("characterDynamics.latestStateEmpty")}
          />
        </div>
      ) : (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/10 p-3 text-xs leading-6 text-muted-foreground">
          {t("characterDynamics.emptyPrompt")}
        </div>
      )}
    </div>
  );
