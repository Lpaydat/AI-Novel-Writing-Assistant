...
export default function ChapterExecutionResultPanel(props: ChapterExecutionResultPanelProps) {
  ...
  if (!selectedChapter) {
    return (
      <div className="rounded-xl border border-dashed p-8 text-sm leading-7 text-muted-foreground">
        {t("result.emptySelect")}
      </div>
    );
  }

  const chapterLabel = t("common.chapterOrderOnly", { order: selectedChapter.order });
  const chapterTitle = selectedChapter.title || t("common.unnamedChapter");
  const chapterObjective = chapterPlan?.objective ?? selectedChapter.expectation ?? t("result.noObjective");
  ...
  const contentPanelTitle = isSelectedChapterFinalizing
    ? t("result.finalizingTitle")
    : useLiveWritingPanel
      ? t("result.liveWritingTitle")
      : t("result.savedTitle");
      ...
      <Card className="h-full overflow-hidden border-border/70">
        <CardContent className="flex h-full min-h-0 flex-col gap-5 pt-5">
          {writingInOtherChapter ? (
            <WorkspaceNotice
              title={t("result.writingInOther.title")}
              description={t("result.writingInOther.description", { label: streamingChapterLabel ?? t("result.anotherChapter") })}
            />
          ) : null}
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[28px] border border-border/80 bg-background shadow-sm">
            <div className="flex flex-col gap-3 border-b bg-muted/20 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={isSelectedChapterStreaming ? "default" : "secondary"}>
                    {isSelectedChapterFinalizing
                      ? t("result.badge.finalizing")
                      : isSelectedChapterStreaming
                        ? t("result.badge.liveWriting")
                        : t("result.badge.saved")}
                  </Badge>
                  <Badge variant="outline">{chapterLabel}</Badge>
                  <Badge variant="outline">{t("result.displayedWordCount", { count: contentPanelWordCount })}</Badge>
                </div>
                <div>
                  <div className="text-base font-semibold text-foreground">{chapterTitle}</div>
                  <div className="mt-1 text-xs leading-6 text-muted-foreground">
                    {t("result.headingJoin", {
                      title: contentPanelTitle,
                      detail: isSelectedChapterFinalizing
                        ? (chapterRunStatus?.message ?? t("result.finalizingDetail"))
                        : isSelectedChapterStreaming
                          ? t("result.liveWritingDetail")
                          : chapterObjective,
                    })}
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs text-muted-foreground">{t("result.wordCount", { count: contentPanelWordCount })}</span>
                {needsAuditPrompt ? (
                  <Button size="sm" onClick={onRunFullAudit} disabled={isRunningFullAudit}>
                    {isRunningFullAudit ? t("result.auditRunning") : t("result.goAudit")}
                  </Button>
                ) : null}
