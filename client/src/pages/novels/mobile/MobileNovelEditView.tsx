...
export default function MobileNovelEditView(props: NovelEditViewProps) {
  ...
  const novelTitle = basicTab.basicForm.title.trim() || t("untitledNovel");
  ...
  const taskAttentionLabel = (() => {
    if (pendingResourceProposalCount > 0) {
      return t("task.resourceCount", { count: pendingResourceProposalCount });
    }
    if (!taskDrawer?.task) {
      return null;
    }
    if (taskDrawer.task.status === "failed") {
      return t("task.abnormal");
    }
    ...
    if (taskDrawer.task.status === "running" || taskDrawer.task.status === "queued") {
      return t("task.inProgress");
    }
    return t("task.recent");
  })();
        ...
        <div className="flex min-w-0 items-start justify-between gap-3">
          ...
          <Dialog open={isToolsOpen} onOpenChange={setIsToolsOpen}>
            <DialogTrigger asChild>
              <Button type="button" size="icon" variant="outline" className="shrink-0" aria-label={t("tools.openAriaLabel")}>
              ...
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-3 gap-2">
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{t("stats.chapters")}</div>
                    <div className="mt-1 font-semibold">{generatedChapters}/{Math.max(totalChapters, 1)}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{t("stats.pendingRepairs")}</div>
                    <div className="mt-1 font-semibold">{pendingRepairs}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{t("stats.task")}</div>
                    <div className="mt-1 truncate font-semibold">{taskAttentionLabel ?? t("common.none")}</div>
                  </div>
                </div>
                ...
                {taskDrawer ? (
                  ...
                  >
                    <span>{t("tools.viewTaskProgress")}</span>
                    {taskAttentionLabel ? <Badge variant="secondary">{taskAttentionLabel}</Badge> : null}
                  </Button>
                ) : null}

                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{t("export.currentStep")}</div>
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    ...
                    >
                      {exportControls.isExportingCurrentMarkdown ? t("export.exporting") : "Markdown"}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    >
                      {exportControls.isExportingCurrentJson ? t("export.exporting") : "JSON"}
                    </Button>
                  </div>
                </div>

                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{t("export.fullBook")}</div>
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    >
                      {exportControls.isExportingFullMarkdown ? t("export.exporting") : "Markdown"}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    >
                      {exportControls.isExportingFullJson ? t("export.exporting") : "JSON"}
                    </Button>
                  </div>
                </div>
              </div>
