...
export default function NovelEdit() {
  const { t, i18n } = useTranslation("novels");
  const { id = "" } = useParams();
  ...
  const exportNovelMutation = useMutation({
    ...
    onSuccess: ({ blob, fileName, scope }) => {
      ...
      toast.success(scope === "full" ? t("edit.export.fullStarted") : t("edit.export.stepStarted"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.export.error"));
    },
  });
  ...
  const volumeSyncPreview = useMemo(
    ...
    [normalizedVolumeDraft, outlineSyncChapters, volumeSyncOptions, i18n.language],
  );
  ...
  const dismissTakeover = () => {
    ...
    toast.success(t("edit.toast.takeoverDismissed"));
  };
  ...
  const continueAutoDirectorMutation = useMutation({
    mutationFn: async (input?: { directorTaskId?: string }) => {
      ...
      if (!targetTaskId) {
        throw new Error(t("edit.error.noContinueTask"));
      }
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.continueFailed");
      toast.error(message);
    },
  });
  const continueAutoExecutionMutation = useMutation({
    mutationFn: async (input?: { directorTaskId?: string; continuationMode?: "auto_execute_range" | "skip_quality_repair" }) => {
      const targetTaskId = input?.directorTaskId || actionTargetDirectorTaskId;
      if (!targetTaskId) {
        throw new Error(t("edit.error.noAutoExecTask"));
      }
      return continueNovelWorkflow(targetTaskId, {
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.continueAutoExecFailed", { scope: activeAutoExecutionScopeLabel });
      toast.error(message);
    },
  });
    ...
    onError: (error, input) => {
      const message = error instanceof Error
        ...
        : input.mode === "auto_execute_range"
          ? t("edit.error.continueAutoExecFailed", { scope: activeAutoExecutionScopeLabel })
          : t("edit.error.continueFailed");
      toast.error(message);
    },
  });
  const executeFollowUpActionMutation = useMutation({
    mutationFn: async (input: {
      ...
      const targetTaskId = input.directorTaskId || actionTargetDirectorTaskId;
      if (!targetTaskId) {
        throw new Error(t("edit.error.noAction"));
      }
    ...
    onSuccess: async (response, input) => {
      ...
      toast.success(result?.message ?? t("edit.toast.actionExecuted"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.error.actionFailed"));
    },
  });
  ...
  const openChapterTitleRepair = (showToast = false) => {
    ...
    toast.success(targetVolumeId ? t("edit.toast.locatedVolumeChapters") : t("edit.toast.switchedToStructured"));
  };
  ...
  const retryAutoDirectorWithCurrentModelMutation = useMutation({
    mutationFn: async () => {
      if (!retryableAutoDirectorTask?.id) {
        throw new Error(t("edit.error.noRetryTask"));
      }
    ...
    onSuccess: async (response) => {
      ...
      toast.success(t("edit.toast.retryWithModel", { provider: llm.provider, model: llm.model }));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.retryWithModelFailed");
      toast.error(message);
    },
  });
  const retryAutoDirectorWithTaskModelMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      void invalidateAutoDirectorTaskState(response.data?.id ?? retryableAutoDirectorTask?.id);
      setIsTaskDrawerOpen(true);
      toast.success(t("edit.toast.retryWithTaskModel"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.retryTaskModelFailed");
      toast.error(message);
    },
  });
  const cancelAutoDirectorMutation = useMutation({
    mutationFn: async (targetTaskId?: string) => {
      ...
      if (!taskId) {
        throw new Error(t("edit.error.noCancelTask"));
      }
    ...
    onSuccess: async (response, targetTaskId) => {
      ...
      toast.success(t("edit.toast.cancelled"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.cancelFailed");
      toast.error(message);
    },
  });
  const archiveCompletedAutoDirectorMutation = useMutation({
    mutationFn: async (targetTaskId?: string) => {
      ...
      if (!taskId) {
        throw new Error(t("edit.error.noArchiveTask"));
      }
    ...
    onSuccess: async (_response, targetTaskId) => {
      ...
      toast.success(t("edit.toast.archived"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("edit.error.archiveFailed");
      toast.error(message);
    },
  });
  useEffect(() => {
  ...
  const takeover = useMemo<NovelEditTakeoverState | null>(() => {
    ...
    const novelTitle = novelDetailQuery.data?.data?.title?.trim() || task.title?.trim() || t("edit.fallbackProjectTitle");
    const reviewScope = activeDirectorSession?.reviewScope ?? null;
    ...
    if (activeChapterTitleWarning) {
      actions.push({
        label: chapterTitleRepairMutation.isPending && chapterTitleRepairMutation.pendingTaskId === task.id
          ? t("edit.action.aiRepairing")
          : activeChapterTitleWarning.label,
        onClick: () => {
          if (hasUnsavedVolumeDraft) {
            toast.error(t("edit.error.unsavedBeforeRepair"));
            return;
          }
    ...
    if (
      ...
      actions.push({
        label: t("edit.action.goConfirmDirection"),
      ...
      actions.push({
        label: t("edit.action.goReviewStage"),
    ...
    if (task.pendingManualRecovery) {
      actions.push({
        label: continueAutoDirectorMutation.isPending ? t("edit.action.continuing") : t("edit.action.continueDirector"),
      ...
      actions.push({
        label: t("edit.action.enterChapterExecution"),
        onClick: () => {
          if (task.resumeTarget?.chapterId) {
            setSelectedChapterId(task.resumeTarget.chapterId);
          }
      ...
      actions.push({
        label: t("edit.action.openQualityRepair"),
    ...
    if (consistencyIssue) {
      actions.push({
        label: continueAutoDirectorMutation.isPending ? t("edit.action.repairing") : t("edit.action.fillDirectorArtifacts"),
        onClick: () => continueAutoDirectorMutation.mutate({ directorTaskId: task.id }),
        variant: "default",
        disabled: continueAutoDirectorMutation.isPending,
      });
      if (consistencyIssue === "missing_characters") {
        actions.push({
          label: t("edit.action.goCharacterSetup"),
    ...
    if (canCancelTask) {
      if (task.status === "failed") {
        actions.push({
          label: cancelAutoDirectorMutation.isPending ? t("edit.action.cancelling") : t("edit.action.cancelTask"),
        ...
        actions.push({
          label: t("edit.action.continueDirecting"),
          onClick: () => setIsDirectorExitActionExpanded(false),
          variant: "outline",
          disabled: cancelAutoDirectorMutation.isPending,
        });
        actions.push({
          label: cancelAutoDirectorMutation.isPending ? t("edit.action.exiting") : t("edit.action.exitDirector"),
          onClick: () => cancelAutoDirectorMutation.mutate(task.id),
          variant: "destructive",
          disabled: cancelAutoDirectorMutation.isPending,
        });
        ...
        actions.push({
          label: t("edit.action.exitDirector"),
      ...
      actions.push({
        label: t("edit.action.dismissReminder"),
        onClick: dismissTakeover,
        variant: "secondary",
      });
    } else if (canArchiveCompletedAutoDirectorTask(task)) {
      actions.push({
        label: archiveCompletedAutoDirectorMutation.isPending ? t("edit.action.archiving") : t("edit.action.completeAndDismiss"),
    ...
    actions.push({
      label: t("edit.action.executionDetails"),
    ...
    return {
      ...
      title: consistencyIssue === "missing_characters"
        ? t("edit.takeover.title.missingCharacters", { title: novelTitle })
        : consistencyIssue === "missing_chapters"
          ? t("edit.takeover.title.missingChapters", { title: novelTitle })
          : task.pendingManualRecovery
            ? t("edit.takeover.title.pendingRecovery", { title: novelTitle })
          : buildTakeoverTitle({
            mode,
            novelTitle,
            checkpointType: task.checkpointType,
      ...
      description: consistencyIssue === "missing_characters"
        ? t("edit.takeover.desc.missingCharacters")
        : consistencyIssue === "missing_chapters"
          ? t("edit.takeover.desc.missingChapters")
          : task.pendingManualRecovery
            ? t("edit.takeover.desc.pendingRecovery")
          : buildTakeoverDescription({
            mode,
            checkpointType: task.checkpointType,
            reviewScope,
      ...
      currentAction: consistencyIssue === "missing_characters"
        ? t("edit.takeover.action.missingCharacters")
        : consistencyIssue === "missing_chapters"
          ? t("edit.takeover.action.missingChapters")
          : task.pendingManualRecovery
            ? (
              task.blockingReason?.trim()
              || task.recoveryHint?.trim()
              || task.lastError?.trim()
              || t("edit.takeover.action.pendingRecovery")
            )
          ...
          : mode === "running" && task.checkpointType === "chapter_batch_ready" && task.currentItemLabel?.includes("已暂停")
            ? t("edit.takeover.action.continuingAutoExec", { scope: autoExecutionScopeLabel })
            : task.currentItemLabel ?? null,
      checkpointLabel: consistencyIssue
        ? t("edit.takeover.checkpoint.pendingArtifacts")
        : task.pendingManualRecovery
          ? t("edit.takeover.checkpoint.pendingRecovery")
        : mode === "running" && task.checkpointType === "chapter_batch_ready"
          ? t("edit.takeover.checkpoint.autoExecuting", { scope: autoExecutionScopeLabel })
  ...
  const taskDrawerActions = useMemo<NovelTaskDrawerState["actions"]>(() => {
    ...
    if (consistencyIssue) {
      actions.push({
        label: continueAutoDirectorMutation.isPending ? t("edit.action.filling") : t("edit.action.fillDirectorArtifacts"),
        onClick: () => continueAutoDirectorMutation.mutate({ directorTaskId: task.id }),
        variant: "default",
        disabled: continueAutoDirectorMutation.isPending,
      });
  ...
  const confirmCharacterResourceProposalMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(t("edit.toast.resourceConfirmed"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.error.resourceConfirmFailed"));
    },
  });
  ...
  const rejectCharacterResourceProposalMutation = useMutation({
    ...
    onSuccess: async () => {
      await invalidateCharacterResourceViews();
      toast.success(t("edit.toast.resourceRejected"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.error.resourceRejectFailed"));
    },
  });
  ...
  const extractChapterResourcesMutation = useMutation({
    mutationFn: async () => {
      if (!selectedChapterId) {
        throw new Error(t("edit.error.noChapterForReview"));
      }
    ...
    onSuccess: async (response) => {
      ...
      if (pendingCount > 0) {
        toast.success(t("edit.toast.reviewedPending", { count: pendingCount }));
        return;
      }
      toast.success(committedCount > 0
        ? t("edit.toast.reviewedCommitted", { count: committedCount })
        : t("edit.toast.reviewedNone"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.error.reviewFailed"));
    },
  });
  ...
  const backfillCharacterResourcesMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      toast.success(pending > 0
        ? t("edit.toast.backfillPending", { scanned, count: pending })
        : t("edit.toast.backfillCommitted", { scanned, count: committed }));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t("edit.error.backfillFailed"));
    },
  });
