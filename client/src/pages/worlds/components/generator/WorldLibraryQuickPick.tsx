  ...
  return (
    <div className="rounded-md border p-3 text-sm space-y-3">
      <div className="space-y-1">
        <div className="font-medium">{t("libraryPick.title")}</div>
      ...
      <input
        ...
        placeholder={t("libraryPick.searchPlaceholder")}
        value={keyword}
      ...
      <div className="space-y-2">
        {libraryQuery.isLoading ? (
          <div className="text-xs text-muted-foreground">{t("libraryPick.loading")}</div>
        ) : null}
            ...
            <div key={item.id} className="flex flex-col gap-3 rounded-md border p-3 md:flex-row md:items-start md:justify-between">
              <div className="space-y-1">
                ...
                <div className="text-xs text-muted-foreground whitespace-pre-wrap">
                  {item.description?.trim() || t("libraryPick.noDescription")}
                </div>
                <div className="text-xs text-muted-foreground">
                  {t("libraryPick.usageCount", { count: item.usageCount })}
                </div>
              </div>
              <Button
                type="button"
                ...
                onClick={() => onAdd(item)}
              >
                {added ? t("libraryPick.added") : t("libraryPick.add")}
              </Button>
            </div>
          );
        })}
