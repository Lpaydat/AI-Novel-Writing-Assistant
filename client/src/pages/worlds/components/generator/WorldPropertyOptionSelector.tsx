              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                    {t(WORLD_LAYER_LABELS[option.targetLayer])}
                  </span>
                  <span className="rounded-full bg-secondary px-2 py-0.5 text-xs text-secondary-foreground">
                    {option.source === "library" ? t("propertyOption.sourceLibrary") : t("propertyOption.sourceSystem")}
                  </span>
                </div>
                <div className="text-muted-foreground">{option.description}</div>
                {option.reason ? (
                  <div className="text-xs text-muted-foreground">
                    {t("propertyOption.reasonPrefix", { reason: option.reason })}
                  </div>
                ) : null}
              </div>
            </label>
              ...
              <div className="space-y-3">
                {option.choices && option.choices.length > 0 ? (
                  <div className="space-y-2 rounded-md border border-dashed p-3">
                    <div className="text-xs font-medium text-muted-foreground">{t("propertyOption.chooseDirection")}</div>
                    <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder={t("propertyOption.detailPlaceholder")}
