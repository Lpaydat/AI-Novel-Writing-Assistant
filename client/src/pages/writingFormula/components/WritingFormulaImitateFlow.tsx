...
export default function WritingFormulaImitateFlow(props: WritingFormulaImitateFlowProps) {
  ...
  return (
    <Card className="border-slate-200/80 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
      <CardHeader>
        <CardTitle>{t("imitate.title")}</CardTitle>
        <div className="text-sm leading-7 text-muted-foreground">
          {t("imitate.description")}
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(300px,0.9fr)]">
          <section className="space-y-3 rounded-2xl border bg-slate-50/70 p-4">
            <div className="text-sm font-medium text-slate-900">{t("imitate.step1Title")}</div>
            <div className="grid gap-3 md:grid-cols-2">
              <input
                className="rounded-md border bg-white p-2 text-sm"
                placeholder={t("imitate.namePlaceholder")}
                value={form.name}
            ...
            <textarea
              ...
              placeholder={t("imitate.sourcePlaceholder")}
            ...
            <div className="flex justify-end">
              <Button type="button" onClick={onExtract} disabled={!form.name.trim() || !form.sourceText.trim() || extractPending}>
                {extractPending ? t("imitate.extracting") : t("imitate.extractBtn")}
              </Button>
            </div>
          </section>

          <section className="space-y-3 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{t("imitate.step2Title")}</div>
            {draft ? (
              <>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {active ? <Badge variant="secondary" className="bg-white/10 text-white">{t("common.currentPlan")}</Badge> : null}
                        </div>
        ...
        <section className="rounded-2xl border bg-white p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-sm font-medium text-slate-900">{t("imitate.step3Title")}</div>
              <div className="mt-1 text-xs leading-6 text-muted-foreground">
                {t("imitate.step3Hint")}
              </div>
            </div>
            <Button type="button" onClick={onCreate} disabled={!draft || createPending}>
              {createPending ? t("imitate.saving") : t("imitate.saveBtn")}
            </Button>
          </div>
          {draft ? (
