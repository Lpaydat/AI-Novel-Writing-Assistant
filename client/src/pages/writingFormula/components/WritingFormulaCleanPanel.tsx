...
export default function WritingFormulaCleanPanel(props: WritingFormulaCleanPanelProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("clean.title")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
        {selectedProfile ? (
          <div className="rounded-2xl border bg-slate-50/70 px-4 py-3 text-sm leading-7 text-slate-700">
            {t("clean.currentProfile", { name: selectedProfile.name })}
          </div>
        ) : (
          <div className="rounded-2xl border bg-slate-50/70 px-4 py-3 text-sm leading-7 text-slate-700">
            {t("clean.selectProfileFirst")}
          </div>
        )}

        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-slate-950">{t("clean.antiAiTitle")}</div>
        ...
        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-slate-950">{t("clean.detectTitle")}</div>
            <div className="text-sm leading-6 text-slate-500">
              {t("clean.detectHint")}
            </div>
          </div>
          ...
          <textarea
            ...
            placeholder={t("clean.detectPlaceholder")}
            value={detectInput}
          ...
          {detectionReport ? (
            <div className="space-y-3 rounded-2xl border p-4 text-sm">
              <div className="space-y-1">
                <div className="font-medium text-slate-900">{t("clean.riskScore", { score: detectionReport.riskScore })}</div>
          ...
          {rewritePreview ? (
            <div className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{t("clean.rewriteResult")}</div>
