...
export default function WritingFormulaSidebar(props: WritingFormulaSidebarProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-y-auto xl:pr-1">
      <Card>
        <CardHeader>
          <CardTitle>{t("sidebar.pickFirstTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="text-sm leading-6 text-muted-foreground">
            {t("sidebar.pickFirstHint")}
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("sidebar.editableAssets")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{profiles.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">
                {t("sidebar.starterCount", { count: starterProfiles.length })}
              </div>
            </div>
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("sidebar.builtinTemplates")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{templates.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">
                {t("sidebar.templateHint")}
              </div>
            </div>
          </div>
          <Button className="w-full" onClick={() => setCreateDialogOpen(true)}>
            {t("sidebar.createOrImport")}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("sidebar.switchTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {customProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("sidebar.yourFormulas")}</div>
          ...
          {starterProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("sidebar.starterFormulas")}</div>
              {starterProfiles.map((profile) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge variant="outline" className="shrink-0">{t("sidebar.starterBadge")}</Badge>
                  </div>
                </button>
              ))}
            </div>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            <DialogTitle>{t("sidebar.createOrImport")}</DialogTitle>
            <DialogDescription>
              {t("sidebar.dialogDesc")}
            </DialogDescription>
          </DialogHeader>

          <Tabs value={activeCreateTab} onValueChange={setActiveCreateTab} className="space-y-4">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="quick_start">{t("sidebar.tab.quickStart")}</TabsTrigger>
              <TabsTrigger value="blank">{t("create.tab.blank")}</TabsTrigger>
              <TabsTrigger value="extract">{t("sidebar.tab.extract")}</TabsTrigger>
            </TabsList>
                ...
                {templates.map((template) => (
                  <div key={template.id} className="rounded-lg border p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge variant="outline">{t("common.templateBadge")}</Badge>
                    </div>
                    ...
                    {template.applicableGenres.length > 0 ? (
                      <div className="mt-3 text-xs text-muted-foreground">
                        {t("common.suitableFor", { genres: template.applicableGenres.join(" / ") })}
                      </div>
                    ) : null}
                    <Button
                      size="sm"
                    ...
                    >
                      {createFromTemplatePending ? t("common.creating") : t("sidebar.createFromTemplate")}
                    </Button>
                  </div>
                ))}
              </div>
            ...
            <TabsContent value="blank" className="space-y-4">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{t("create.manualTitle")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">
                      {t("sidebar.manualDesc")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={t("sidebar.manualNamePlaceholder")}
                    ...
                    >
                      {createManualPending ? t("common.creating") : t("create.createBlank")}
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{t("create.aiTitle")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">
                      {t("sidebar.aiDesc")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <textarea
                      ...
                      placeholder={t("sidebar.briefPlaceholder")}
                    ...
                    >
                      {createFromBriefPending ? t("create.aiGenerating") : t("create.aiGenerate")}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border p-4">
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    placeholder={t("sidebar.extractPlaceholder")}
                  ...
                  >
                    {extractFromTextPending ? t("sidebar.extracting") : t("sidebar.extractBtn")}
                  </Button>
                </div>
              </div>
            </TabsContent>
