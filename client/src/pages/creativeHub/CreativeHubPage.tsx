...
export default function CreativeHubPage() {
  ...
  const handleCreateNovelQuickAction = useCallback(async (title: string) => {
    ...
    await runtimeState.sendPrompt(t("page.prompt.createNovel", { title: normalized }));
  }, [runtimeState, t]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="secondary">{t("hub.title")}</Badge>
        {currentThread ? <Badge variant="outline">{currentThread.title}</Badge> : null}
        {currentBindings.novelId ? <Badge variant="outline">{t("page.badge.novel", { id: currentBindings.novelId })}</Badge> : null}
        {currentBindings.worldId ? <Badge variant="outline">{t("page.badge.world", { id: currentBindings.worldId })}</Badge> : null}
        {currentBindings.taskId ? <Badge variant="outline">{t("page.badge.task", { id: currentBindings.taskId })}</Badge> : null}
        {currentBindings.bookAnalysisId ? <Badge variant="outline">{t("page.badge.bookAnalysis", { id: currentBindings.bookAnalysisId })}</Badge> : null}
        {currentBindings.formulaId ? <Badge variant="outline">{t("page.badge.formula", { id: currentBindings.formulaId })}</Badge> : null}
        {currentBindings.baseCharacterId ? <Badge variant="outline">{t("page.badge.character", { id: currentBindings.baseCharacterId })}</Badge> : null}
        {currentBindings.styleProfileId ? <Badge variant="outline">{t("page.badge.style", { id: currentBindings.styleProfileId })}</Badge> : null}
        ...
        {currentBindings.knowledgeDocumentIds?.length ? (
          <Badge variant="outline">{t("page.badge.knowledgeDocs", { count: currentBindings.knowledgeDocumentIds.length })}</Badge>
        ) : null}
        {currentCheckpointId ? (
      ...
      <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
        ...
        <div>
          <div className="font-medium">{t("page.notice.title")}</div>
          <div className="mt-0.5 leading-6">{t("page.notice.body")}</div>
        </div>
      </div>
