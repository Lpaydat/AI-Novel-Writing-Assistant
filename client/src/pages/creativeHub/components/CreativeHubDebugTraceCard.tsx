  ...
  return (
    <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-sm font-medium text-slate-900">{t("debugTrace.title")}</div>
          <div className="mt-1 text-xs text-slate-500">
            {runId ? `Run ${runId.slice(0, 8)}` : t("debugTrace.currentTurnDebug")} · {t("debugTrace.entryCount", { count: entries.length })}
          </div>
        </div>
        <button
          type="button"
        ...
        >
          {expanded ? t("debugTrace.collapseDetails") : t("debugTrace.expandDetails")}
        </button>
      </div>
      {expanded ? (
