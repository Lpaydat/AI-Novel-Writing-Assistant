...
export default function CreativeHubInlineToolCall(props: ToolCallMessagePartProps) {
  ...
  if (props.toolName === "approval_gate") {
    const title = typeof args.title === "string" ? args.title : t("status.waitingApproval");
    const summary = typeof args.summary === "string" ? args.summary : t("inlineTool.highImpactSummary");
    const targetType = typeof args.targetType === "string" ? args.targetType : inlineControls.interrupt?.targetType ?? t("inlineTool.unknownTarget");
    ...
    return (
      <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
        ...
        <textarea
          ...
          placeholder={t("inlineTool.approvalNotePlaceholder")}
        />
    ...
    <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="space-y-1">
          <div className="text-sm font-medium text-slate-900">{t("inlineTool.toolCall", { name: props.toolName })}</div>
        ...
        <div className="flex items-center gap-2">
          {argsText ? (
            ...
            >
              {showArgs ? t("inlineTool.collapseArgs") : t("inlineTool.viewArgs")}
            </button>
          ) : null}
      ...
      {argsText && showArgs ? (
        ...
        <div className="mt-2 text-xs text-slate-500">{t("inlineTool.argsCollapsedHint")}</div>
      ) : null}
      {(resultText || artifact.summary) ? (
        <div className="mt-3">
          <CreativeHubToolResultCard
            toolName={props.toolName}
            summary={artifact.summary ?? resultText ?? t("common.toolReturned")}
            success={success}
