    ...
    >
      <AssistantRuntimeProvider runtime={runtime}>
        <Card className="flex h-full min-h-0 flex-col">
          <CardHeader>
            <CardTitle className="text-base">{t("hub.title")}</CardTitle>
          </CardHeader>
              ...
              <ThreadPrimitive.Viewport className="min-h-0 flex-1 space-y-4 overflow-y-auto rounded-2xl bg-gradient-to-b from-slate-50 to-slate-100/70 p-4 ring-1 ring-slate-200">
                <ThreadPrimitive.Empty>
                  <div className="mx-auto mt-8 max-w-[680px] px-2 text-center">
                    <h3 className="text-4xl font-semibold tracking-tight text-slate-900">{t("hub.title")}</h3>
                    <p className="mt-2 text-lg text-slate-500">{t("conversation.emptySubtitle")}</p>
                    {onQuickAction ? (
                      <div className="mt-5 flex flex-wrap justify-center gap-2">
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("conversation.qa.direction.prompt"))}>
                          {t("conversation.qa.direction.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("conversation.qa.diagnose.prompt"))}>
                          {t("conversation.qa.diagnose.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("conversation.qa.promise.prompt"))}>
                          {t("conversation.qa.promise.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("conversation.qa.converge.prompt"))}>
                          {t("conversation.qa.converge.label")}
                        </Button>
                      </div>
                    ) : null}
                  </div>
