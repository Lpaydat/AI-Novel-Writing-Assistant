    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-end">
        <div className="space-y-1">
          <label className="text-sm font-medium">{t("export.selectEpisode")}</label>
          <SelectControl
          ...
          >
            {episodes.map((ep) => (
              <option key={ep.id} value={ep.id}>
                {t("export.episodeOption", {
                  order: ep.order,
                  title: ep.title ? t("episode.titleQuoted", { title: ep.title }) : "",
                  count: ep._count?.panels ?? 0,
                })}
              </option>
            ))}
          </SelectControl>
        ...
        >
          ...
          {exportMut.isPending ? t("export.exporting") : t("export.exportLongImage")}
        </Button>
      </div>
...
const STYLE_OPTIONS = [
  { value: "webtoon_color", label: "styleOption.webtoon_color.label", desc: "styleOption.webtoon_color.desc" },
  { value: "bl_manga", label: "styleOption.bl_manga.label", desc: "styleOption.bl_manga.desc" },
  { value: "shounen_bw", label: "styleOption.shounen_bw.label", desc: "styleOption.shounen_bw.desc" },
  { value: "ink_traditional", label: "styleOption.ink_traditional.label", desc: "styleOption.ink_traditional.desc" },
  { value: "chibi", label: "styleOption.chibi.label", desc: "styleOption.chibi.desc" },
  { value: "realistic", label: "styleOption.realistic.label", desc: "styleOption.realistic.desc" },
];
...
export default function ComicProjectPage() {
  ...
  // The image-model choice persists across projects/refreshes (users usually stick with one image model).
  ...
  // Use the cached provider only if it still exists in the available list; otherwise fall back to the first (avoids referencing a stale provider config).
  ...
  if (!project) {
    return <div className="p-8 text-center text-muted-foreground">{t("project.notFound")}</div>;
  }
  ...
  const statusLabel: Record<string, string> = {
    draft: t("projectStatus.draft"), outlined: t("projectStatus.outlined"), scripted: t("projectStatus.scripted"), completed: t("projectStatus.completed"),
  };
  const sourceLabel: Record<string, string> = {
    novel_import: t("sourceLabel.novel_import"), original: t("sourceLabel.original"), text_import: t("sourceLabel.text_import"), comic_import: t("sourceLabel.comic_import"),
  };
    ...
    <div className="w-full space-y-5 px-4 py-6 lg:px-6">
      ...
      <div className="rounded-xl border bg-card p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {/* Format card — click to expand the picker */}
          <div className="relative">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-xs text-muted-foreground">{t("format.heading")}</p>
                <p className="text-sm font-semibold">{t(formatDef.label)}</p>
                <p className="text-[10px] text-muted-foreground leading-tight max-w-[100px]">{t(formatDef.desc)}</p>
              </div>
            ...
            {showFormatPicker && (
              <div className="absolute right-0 top-full mt-2 z-50 w-[480px] rounded-xl border bg-popover shadow-xl p-4">
                <p className="text-xs font-medium text-muted-foreground mb-3">{t("format.pickerTitle")}</p>
                <div className="grid grid-cols-4 gap-2">
                  {COMIC_FORMATS.map((fmt) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-medium">{t(fmt.label)}</span>
                      <span className="text-[10px] text-muted-foreground leading-tight">{t(fmt.tag)}</span>
                    </button>
                  ))}
                </div>
                <button
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="flex items-center gap-2.5 rounded-lg border bg-background px-3 py-2.5">
            ...
            <div>
              <p className="text-[11px] text-muted-foreground">{t("stats.episodes")}</p>
          ...
          <div className="flex items-center gap-2.5 rounded-lg border bg-background px-3 py-2.5">
            ...
            <div>
              <p className="text-[11px] text-muted-foreground">{t("stats.totalPanels")}</p>
          ...
          <div className="flex items-center gap-2.5 rounded-lg border bg-background px-3 py-2.5">
            ...
            <div>
              <p className="text-[11px] text-muted-foreground">{t("stats.characters")}</p>
              <p className="text-lg font-bold leading-tight">{project._count?.characters ?? project.characters.length}</p>
            </div>
          </div>

          {/* Art style — click to expand the picker */}
          <div className="relative">
            <button
              type="button"
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                <p className="text-[11px] text-muted-foreground">{t("style.heading")}</p>
                <p className="text-sm font-semibold leading-tight truncate">
                  {styleDef ? t(styleDef.label) : (preset.style ?? t("style.default"))}
                </p>
              </div>
            ...
            {showStylePicker && (
              <div className="absolute right-0 top-full mt-2 z-50 w-64 rounded-xl border bg-popover shadow-xl p-3">
                <p className="text-xs font-medium text-muted-foreground mb-2">{t("style.pickerTitle")}</p>
                <div className="space-y-1">
                  {STYLE_OPTIONS.map((opt) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="ml-2 text-xs text-muted-foreground">{t(opt.desc)}</span>
                      </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-0.5">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap">{t("imageModel.label")}</span>
            {providerOptions.length === 0 ? (
              <span className="text-xs text-destructive">{t("imageModel.noProviders")}</span>
            ) : (
              <SelectControl
      ...
      <Tabs defaultValue="outline">
        <TabsList className="w-full justify-start gap-1">
          <TabsTrigger value="outline">{t("tab.outline")}</TabsTrigger>
          ...
          <TabsTrigger value="scenes">{t("tab.scenes")}</TabsTrigger>
          <TabsTrigger value="panels">{t("tab.panels")}</TabsTrigger>
