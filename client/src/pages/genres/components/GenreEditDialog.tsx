...
export default function GenreEditDialog({
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      if (!genre) {
        throw new Error(t("edit.notFoundError"));
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        footer={(
          <>
            ...
            <Button type="button" onClick={() => updateMutation.mutate()} disabled={updateMutation.isPending || !name.trim()}>
              {updateMutation.isPending ? t("common.saving") : t("edit.saveButton")}
            </Button>
          </>
        )}
      ...
      >
        <div className="space-y-4">
          ...
          <label className="space-y-2 text-sm">
            <span className="font-medium text-foreground">{t("field.parent")}</span>
            <SelectControl
              className="w-full rounded-md border bg-background p-2 text-sm"
              value={parentId}
              onChange={(event) => setParentId(event.target.value)}
            >
              <option value="">{t("edit.rootOption")}</option>
