...
export default function GenreManagementPage() {
  ...
  const handleDelete = (genre: GenreTreeNode) => {
    ...
    const message = descendantCount > 0
      ? t("confirm.deleteWithChildren", { name: genre.name, count: descendantCount })
      : t("confirm.delete", { name: genre.name });
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="text-sm text-muted-foreground">{t("page.count", { count: totalGenres })}</div>
            <Button type="button" onClick={handleCreateRoot}>
              {t("button.createRootTree")}
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {genreTreeQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{t("state.loading")}</div>
          ) : null}

          {!genreTreeQuery.isLoading && genreTree.length === 0 ? (
            <div className="rounded-xl border border-dashed p-6 text-center">
              <div className="text-sm font-medium text-foreground">{t("empty.title")}</div>
