  ...
  return (
    <div className={`grid gap-3 ${showProtocolFields ? "md:grid-cols-6" : "md:grid-cols-4"}`}>
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{t("fields.provider")}</div>
        <Select
        ...
        >
          <SelectTrigger>
            <SelectValue placeholder={t("fields.selectProvider")} />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{t("fields.model")}</div>
        <SearchableSelect
          value={draft.model || undefined}
          onValueChange={(value) => onPatch({ model: value })}
          options={modelOptions.map((model) => ({ value: model }))}
          placeholder={t("fields.selectModel")}
          searchPlaceholder={t("fields.searchModel")}
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{t("fields.temperature")}</div>
        <Input
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{t("fields.maxTokens")}</div>
        <Input
      ...
      {showProtocolFields ? (
        <div className="space-y-1">
          <div className="text-xs text-muted-foreground">{t("fields.requestProtocol")}</div>
          <Select
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder={t("fields.auto")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="auto">{t("fields.auto")}</SelectItem>
              <SelectItem value="openai_compatible">{t("fields.openaiCompatible")}</SelectItem>
      ...
      {showProtocolFields ? (
        <div className="space-y-1">
          <div className="text-xs text-muted-foreground">{t("fields.structuredFormat")}</div>
          <Select
            ...
            <SelectContent>
              {getStructuredResponseFormatOptions(draft.requestProtocol).map((format) => (
                <SelectItem key={format} value={format}>
                  {format === "auto" ? t("fields.auto") : format}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
