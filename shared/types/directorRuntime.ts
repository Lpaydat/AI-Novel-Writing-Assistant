...
const DIRECTOR_NODE_DISPLAY_LABELS: Record<Locale, Record<string, string>> = {
  zh: {
  ...
  en: {
    candidate_generation: "Generate book-level directions",
    candidate_refine: "Refine book-level directions",
    candidate_patch: "Patch book-level directions",
    candidate_title_refine: "Refine book title",
    novel_create: "Create novel project",
    takeover_execution: "Take over existing project",
    story_macro: "Story macro planning",
    story_macro_phase: "Story macro planning",
    book_contract: "Book creation contract",
    book_contract_phase: "Book creation contract",
    character_setup: "Character cast setup",
    character_setup_phase: "Character cast setup",
    volume_strategy: "Volume strategy",
    volume_strategy_phase: "Volume strategy",
    "volume_strategy.volume_generation": "Generate volume strategy",
    structured_outline: "Chapter breakdown & task sheet",
    structured_outline_phase: "Chapter breakdown & task sheet",
    "structured_outline.beat_sheet": "Generate beat sheet",
    "structured_outline.chapter_list": "Generate chapter list",
    ...
    "book.candidate.generate": "Generate book-level directions",
    "book.candidate.refine": "Refine book-level directions",
    "book.candidate.patch": "Patch book-level directions",
    "book.candidate.title_refine": "Refine book title",
    "book.project.create": "Create novel project",
    "workflow.takeover.execute": "Take over existing project",
    "story.macro.plan": "Story macro planning",
    "book.contract.create": "Book creation contract",
    "character.cast.prepare": "Character cast setup",
    "volume.strategy.plan": "Volume strategy",
    "chapter.task_sheet.plan": "Chapter breakdown & task sheet",
    ...
    chapter_state_commit: "Update chapter state",
    chapter_state_commit_node: "Update chapter state",
    payoff_ledger_sync: "Sync foreshadowing & reader promises",
    payoff_ledger_sync_node: "Sync foreshadowing & reader promises",
    character_resource_sync: "Sync character state",
    character_resource_sync_node: "Sync character state",
    "chapter.draft.write": "Chapter draft writing",
    "planner.chapter.plan": "Chapter planning",
    "novel.chapter.writer": "Chapter draft writing",
    "audit.chapter.light": "Basic quality review",
    "audit.chapter.full": "Full quality review",
    "novel.review.patch": "Localized text repair",
    "style.detection": "Style check",
    "style.rewrite": "Style adjustment",
    "novel.payoff_ledger.sync": "Foreshadowing & reader promise sync",
    "novel.characterDynamics.chapterExtract": "Character dynamics sync",
    "novel.character_resource.extract_updates": "Character resource sync",
    "state.snapshot.extract": "Chapter state sync",
    "chapter.quality.review": "Chapter quality review",
    "chapter.draft.repair": "Chapter issue repair",
    "chapter.state.commit": "Update chapter state",
    "payoff.ledger.sync": "Sync foreshadowing & reader promises",
    "character.resource.sync": "Sync character state",
    "planner.replan": "Adjust subsequent chapter planning",
  },
};

const DIRECTOR_NODE_FALLBACK_LABEL: Record<Locale, string> = {
  zh: "AI 推进步骤",
  en: "AI Advance Step",
};
...
export function getDirectorNodeDisplayLabel(input: {
  ...
  const labels = DIRECTOR_NODE_DISPLAY_LABELS[locale];
  const label = input.label?.trim() ?? "";
  const nodeKey = input.nodeKey?.trim() ?? "";
  const mappedLabel = label ? labels[label] : null;
  if (mappedLabel) {
    return mappedLabel;
  }
  const mappedNode = nodeKey ? labels[nodeKey] : null;
  if (mappedNode) {
    return mappedNode;
  }
  ...
  return input.fallback ?? DIRECTOR_NODE_FALLBACK_LABEL[locale];
}
